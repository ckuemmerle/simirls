%% Load data for Figure 4a (upper row) 
% This script is loads the experiment data for Figure 4 (upper row) in 
% the paper [1], cf. Appendix.
% Note: The corresponding experiment can be reproduced by using the script
% 'SimIRLS_run_experiment_Figure4a.m'.
...
ms_all = 40:5:280;
sparsitys=[1,2:2:40];
load('LRRS_m_45_155_LRRS_GRank1_rank1_n2_40_Gaussian_rank1_K1_1_2023-05-21_0220-22-333','Example')
...
for ii = 1:length(sparsitys)
    exp_part1 = 'LRRS_GRank1_rank1_n2_40_Gaussian_rank1_';
...
for i=1:length(Example.alg_name_out)
    ...
    ax= gca;
    ax.YDir = 'normal';
    title(strcat([Example.alg_name_out{i},', Random rank-one meas.']),'interpreter','latex');
    hold on;
    ...
    %% Save file
    filename=strcat('results/experiment_GR1Meas_n256_40_rank1_',Example.alg_name{i});
    save_LRRS_plots;
end