%% Load data for Figure 4b (lower row) 
% This script is loads the experiment data for Figure 4 (lower row) in 
% the paper [1], cf. Appendix.
% Note: The corresponding experiment can be reproduced by using the script
% 'SimIRLS_run_experiment_Figure4b.m'.
...
sparsitys=[10:5:60];
load('LRRS_m_300_875_LRRS_GRank1_rank5_n2_40_Gaussian_rank1_K1_10_2023-05-21_1422-16-129','Example')
...
for ii = 1:length(sparsitys)
    exp_part1 = 'LRRS_GRank1_rank5_n2_40_Gaussian_rank1_';
...
for i=1:length(Example.alg_name_out)
    ...
    ax= gca;
    ax.YDir = 'normal';
    title(strcat([Example.alg_name_out{i},', Random rank-1 meas.']),'interpreter','latex');
    hold on;
    ...
    %% Save file
    filename=strcat('results/experiment_GR1Meas_n256_40_rank5_',Example.alg_name{i});
    save_LRRS_plots;
end