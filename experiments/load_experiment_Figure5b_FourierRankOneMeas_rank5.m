%% Load data for Figure 5b (lower row) 
% This script is loads the experiment data for Figure 5 (lower row) in 
% the paper [1], cf. Appendix.
% Note: The corresponding experiment can be reproduced by using the script
% 'SimIRLS_run_experiment_Figure5b.m'.
...
sparsitys=[10:5:60];
load('LRRS_m_300_650_LRRS_FourierRank1_rank5_n2_40_Fourier_rank1_K1_10_2023-05-23_1252-50-320','Example')
...
for ii = 1:length(sparsitys)
    exp_part1 = 'LRRS_FourierRank1_rank5_n2_40_Fourier_rank1_';
    ...
    elseif length(Dir) == 0
        ...
        successrate = zeros(length(Example.alg_name),len);
    else
...
for i=1:length(Example.alg_name_out)
    ...
    %% Save file
    filename=strcat('results/experiment_FourierR1Meas_n256_40_rank5_',Example.alg_name{i});
    save_LRRS_plots;
end