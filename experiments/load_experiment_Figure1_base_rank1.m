...
% This script is loads the experiment data for Figure 1 in the paper
% [1], cf. Section 4.
% Note: The corresponding experiment can be reproduced by using the script
% 'SimIRLS_run_experiment_Figure1.m'.
...
ms_all = 40:5:280;
sparsitys=[1,2:2:40];
load('LRRS_m_45_155_LRRS_Gaussian_rank1_n2_40_Gaussian_dense_K1_1_2023-05-14_1233-31-041','Example') % load first data file
disp(['Possible measurements: ',num2str(ms_all)])
successrate_matrix = zeros(length(sparsitys),length(ms_all),length(Example.alg_name));
for ii = 1:length(sparsitys) % load data files for larger row sparsity values than 1
    exp_part1 = 'LRRS_Gaussian_rank1_n2_40_Gaussian_dense_';
...
%     title('')
    ...
    %% Save file
    filename=strcat('results/experiment_Gauss_n256_40_rank1_base_',Example.alg_name{i});
    save_LRRS_plots;
end
nr_plots = 2;
plot_options = struct;
...
title('')
%% Save file
filename=strcat('results/experiment_GC_ER_good_PT_adaptive_T_7_oversampling');
save_LRRS_plots;